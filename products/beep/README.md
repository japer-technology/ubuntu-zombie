...
<p align="center">
  <picture>
    <img src="https://raw.githubusercontent.com/japer-technology/ubuntu-zombie/main/LOGO.png" alt="Beep" width="500">
  </picture>
</p>

## An AI System Administrator

> **Beep adds a private, root-capable AI Systems
...
# after reboot:
/opt/beep/bin/verify
sudo /opt/beep/bin/beep-secrets-edit   # add an LLM API key
...
the chat — the beep permanently disables itself. Routine reinstalls
...
GitHub and npm endpoints for the latest Beep and bridge releases.
...
`/opt/beep/secrets/env`, not from `pi`'s native `~/.pi` defaults:
set exactly one matching `*_API_KEY`, optionally set
`BEEP_PROVIDER`, and set `BEEP_MODEL` when you need a non-default
...
`BEEP_MODEL` or a provider-specific model override is already configured
in the environment or installed `secrets/env`; skip it explicitly with
`BEEP_SKIP_LLM_SCAN=1`. See
...
ships `beep_<version>_all.deb` plus a `SHA256SUMS` checksum file
...
The `beep` wrapper then accepts the same subcommands as
...
`uninstall`. Public component targets are `beep`, `forgejo`,
`forgejo-runner`, and `llama`. With
no target, `install` keeps its existing meaning: install or upgrade the
`beep` baseline. Examples:

```bash
sudo ./scripts/install.sh install             # baseline beep
sudo ./scripts/install.sh install beep      # same, explicit target
...
create the beep account, install Node or the
...
`forgejo` dependency. `BEEP_INSTALL_FORGEJO=1 install` remains supported
and selects the legacy combined `beep forgejo` path.
...
`BEEP_INSTALL_*` flags — all off by default, idempotent, audited,
...
[`docs/CONFIGURATION.md`](docs/CONFIGURATION.md#optional-components-beep--options).
...
local users only; it never listens on the LAN. Use `beep-llama-manager` to
start, stop, restart, enable, disable, test, or inspect it. Removing
Beep leaves this component untouched.
...
The local `beep` Linux user (renameable at install time with
`BEEP_USER=<name>`) is the operating identity of the AI
...
Beep is released under the MIT License. By contributing you agree
