...
Beep is a local-only AI Systems Administrator for Ubuntu
...
flowchart TD
    ...
    installer --> overlay["/etc/beep/<br/>operator-editable policy/skills overlays"]
...
`127.0.0.1:${BEEP_CHAT_PORT:-7878}`; the optional standalone llama
...
Built-in skills ship under `/opt/beep/skills/` and cover `ai-agents`,
...
`ubuntu`, `users`, `virtualization`, `web`, `beep` and `zram`.
...
briefs under `/etc/beep/skills.d/`.
...
<verb> [component ...] [flags]`. Public targets currently are `beep`
...
runner. The legacy `BEEP_INSTALL_*` flags remain supported and are
...
(`BEEP_INSTALL_FORGEJO`): a git forge backed by PostgreSQL, running
as the dedicated `git` system user under a hardened `forgejo.service`
unit, plus an optional co-located Actions runner
(`BEEP_INSTALL_FORGEJO_RUNNER`, restricted Docker executor,
...
`install_beep` converges the account, runtimes, policy, and chat stack;
...
does not depend on beep-owned state.

The standalone **llama** component has no dependency on `beep`. It
...
never LAN-facing. `/usr/local/bin/beep-llama-manager` is the lifecycle and
...
| Verb | Behaviour |
| ---- | --------- |
| `install` | Idempotent install. With no target, selects `beep`. |
...
create neither the beep account nor `/opt/beep`, and they do not
...
`/var/lib/beep/components/`. This directory is intentionally
outside `/opt/beep`, so selective beep removal does not erase the
manifest entry for a remaining component such as Forgejo.

Manifest files use a fixed format-version-`1` key/value layout:
`format=`, `component=`, `beep_version=`, `converged_utc=`,
...
| Path | Purpose |
| ---- | ------- |
| `/var/log/beep-install.log` | Installer transcript. |
| `/var/log/beep/install-receipt.txt` | Non-secret install receipt. |
| `/var/log/beep/audit.log` | JSON-lines audit trail. |
| `/opt/beep/state/conversations.db` | Chat history. |
| `/opt/beep/state/lifecycle.json` | TTL/tombstone state. |
| `/opt/beep/state/logs/` | pi-mono bridge logs. |
