...
Provider credentials live in `/opt/beep/secrets/env`, mode `0600`,
owned by the local agent account (default `beep:beep`; whatever
name was passed to `BEEP_USER` at install time). Edit them with the
...
| `OPENROUTER_API_KEY` | API key for the OpenRouter aggregator. Requires `BEEP_MODEL` to be set to a fully-qualified id such as `anthropic/claude-3.5-sonnet`. |
| `MISTRAL_API_KEY`    | API key for the Mistral provider         |
| `GROQ_API_KEY`       | API key for the Groq provider            |
| `BEEP_PROVIDER`    | One of `openai`, `anthropic`, `gemini`, `xai`, `mistral`, `groq`, `openrouter`, `lmstudio` (default: first matching key found, in that order) |
| `BEEP_MODEL`       | Model used by both the agent loop and the chat surface; required for `openrouter`/`lmstudio` unless their provider-specific model env var is set; overrides provider-specific model env vars and defaults |
| `BEEP_OPENAI_MODEL`     | Override the default model used when the active provider is `openai` |
| `BEEP_ANTHROPIC_MODEL`  | Override the default model used when the active provider is `anthropic` |
| `BEEP_GEMINI_MODEL`     | Override the default model used when the active provider is `gemini` |
| `BEEP_XAI_MODEL`        | Override the default model used when the active provider is `xai` |
| `BEEP_MISTRAL_MODEL`    | Override the default model used when the active provider is `mistral` |
| `BEEP_GROQ_MODEL`       | Override the default model used when the active provider is `groq` |
| `BEEP_OPENROUTER_MODEL` | Fully-qualified OpenRouter model id (e.g. `anthropic/claude-3.5-sonnet`); used only when `BEEP_MODEL` is unset |
| `BEEP_CHAT_PORT`   | Loopback port for the chat UI (default `7878`) |
| `BEEP_ADMIN_PASSWORD` | Chat-UI password gate. The installer asks for it (default `braaaains`) and stores only a PBKDF2 hash as `BEEP_ADMIN_PASSWORD_HASH` in `secrets/env`. |
| `BEEP_TTL_DAYS`    | Initial Time to Live in whole days before the beep is permanently disabled (default `7`). Valid existing lifecycle state is preserved on reinstall. |
| `LMSTUDIO_API_KEY`   | API key for a local OpenAI-compatible server (LM Studio / Ollama / llama.cpp). Pair with `BEEP_PROVIDER=lmstudio` and `BEEP_MODEL`; the server URL lives in `~/.pi/agent/models.json` (most local servers ignore the key). |
...
Per-provider defaults if no `BEEP_MODEL` / `BEEP_<PROVIDER>_MODEL`
...
`/opt/beep/agent/pi-ai.version`. The chat service shells out to
the Node bridge at `/opt/beep/agent/pi-ai-bridge.mjs`; there are
no bespoke per-provider Python clients.

`BEEP_PROVIDER` + `BEEP_MODEL` (plus the matching `*_API_KEY`) are
...
1. explicit provider argument (internal API only), else
   `BEEP_PROVIDER`, else the first configured key in the provider
   table order above;
2. explicit model argument (internal API only), else `BEEP_MODEL`,
   else the provider-specific `BEEP_<PROVIDER>_MODEL`, else the
   registry default.
...
key; set `BEEP_LOCAL_LLM_API_KEY` (or edit the files afterwards) if
...
current browser; `/rebrand` resets those labels to Beep.
...
fixed GitHub and npm metadata endpoints for current Beep, `pi-mono`,
...
| ------------------------ | ------- | -------------------------------------------------------------------- |
| `BEEP_SKIP_LLM_SCAN`   | `0`     | Set to `1` to skip the LAN scan entirely.                            |
| `BEEP_LLM_SCAN_PORT`   | `1234`  | Extra TCP port probed by runtime `/locals`; installer discovery uses this port exclusively. |
| `BEEP_LOCAL_LLM_API_KEY` | `local` | API key recorded for the discovered server (most local servers ignore it). |
...
`beep`. To pick a different name, pass `BEEP_USER` to the
installer:

```bash
sudo BEEP_USER=admin ./scripts/install.sh install
```
...
drop-in `/etc/sudoers.d/90-<name>-beep`, the systemd
`User=`/`Group=` of `beep-chat.service`, and the system
prompt the chat service hands to the LLM.

## Rotating provider keys

1. `sudo /opt/beep/bin/beep-secrets-edit` — replace the value.
2. `sudo systemctl restart beep-chat.service`.
...
```bash
sudo /opt/beep/bin/beep-secrets-edit   # delete every API key
sudo systemctl restart beep-chat.service
```
...
```bash
sudo systemctl disable --now beep-chat.service
```

To remove privileged access without uninstalling everything:

```bash
sudo rm /etc/sudoers.d/90-beep-beep
```

## Policy

`/etc/beep/policy.yaml` controls what the agent may run
...
allow-list (`/opt/beep/state`, `/etc`, `/var/log`, `/proc`, `/sys`,
...
home directories and `/opt/beep/secrets` stay out of reach.
...
`fs.write` remains limited to `/opt/beep/state` and `/tmp`.
...
can still tune all three values in `/opt/beep/etc/policy.yaml`; elevated
...
`/opt/beep/agent/pi-mono.version`. It renders runtime configs into
`/opt/beep/pi/`:

| Path                                   | Purpose                                  |
| -------------------------------------- | ---------------------------------------- |
| `/opt/beep/pi/settings.json`      | pi-mono settings (`--no-builtin-tools`)  |
| `/opt/beep/pi/APPEND_SYSTEM.md`   | rendered system-prompt prelude           |
| `/opt/beep/agent/pi-mono-bridge.mjs` | Node bridge wrapping `pi --mode json` |
| `/opt/beep/state/logs/pi-mono.*.log` | per-turn bridge logs, rotated daily   |
| `/opt/beep/state/pi-mono-sessions/`  | pi session/checkpoint state           |
...
below (look for the `BEEP_PI_MONO_*` variables).

## Chat access

The chat UI is served at `http://127.0.0.1:${BEEP_CHAT_PORT:-7878}/`.
...
`BEEP_ADMIN_PASSWORD_HASH` in `secrets/env`. Set a custom one with
`BEEP_ADMIN_PASSWORD` or through the interactive parameter review.
...
`<beep-reactivation>` request in the agent's reply. The server removes
...
**Time to Live** defaults to 7 days (`BEEP_TTL_DAYS`, or set it in the
interactive review). When the TTL elapses — or an operator runs the
`/ttl --die` chat command — the beep writes a durable tombstone and is
**permanently disabled**: it refuses to answer prompts and shows a "this
beep has died" notice. A re-run of `scripts/install.sh install` preserves a
...
| `/ttl --die`             | Trip the kill switch now — permanently disables the beep.    |
...
State lives in `/opt/beep/state/lifecycle.json`. It can also be
inspected from the agent account with
`python3 /opt/beep/agent/lifecycle.py status`.

## Logs and state

| Path                                       | Purpose                                         |
| ------------------------------------------ | ----------------------------------------------- |
| `/var/log/beep-install.log`       | Installer transcripts                           |
| `/var/log/beep/install-receipt.txt` | Install receipt (parameters + start/finish outcome) |
| `/var/log/beep/audit.log`         | JSON-lines AI audit trail                       |
| `/opt/beep/state/conversations.db`    | Chat history (SQLite)                           |
| `/opt/beep/state/lifecycle.json`      | Time-to-Live state + tombstone                  |
| `/opt/beep/state/logs/pi-mono.*.log`  | Per-turn pi-mono bridge logs (rotated daily)    |
| `/opt/beep/state/pi-mono-sessions/`   | pi session/checkpoint state                     |
...
| Command                | Purpose                                                                 |
| ---------------------- | ----------------------------------------------------------------------- |
| `beep-secrets-edit`         | Safely edit `secrets/env`; re-asserts `0600` mode after `$EDITOR` exits |
| `beep-health`         | One-shot health summary (chat service, provider credential, disk, …)    |
| `beep-audit`         | Tail the most recent decisions from `audit.log`                         |
| `beep-diagnostics`  | Bundle logs and state into a tarball with secrets redacted              |
| `beep-chat`          | Print the local chat URL                                               |
...
not `--yes` and not `BEEP_NONINTERACTIVE=1`), it opens an editable
**parameter review** before touching the host. The review is scoped to
the selected components. Beep runs show agent, chat, TTL, provider, and
...
The review uses the **Beep Orchid** highlight (`#AC43D9`) with
compatible accent colours when colour is enabled. Colour follows the same
`BEEP_COLOR=auto|always|never` / `NO_COLOR` policy as the rest of the
output, so `--no-color` produces a plain, screen-reader-friendly table.

Automated runs (`--yes`, `BEEP_NONINTERACTIVE=1`, or non-TTY stdin) skip
...
[flags]`; public component targets are `beep`, `forgejo`,
`forgejo-runner`, and `llama`. Existing `BEEP_INSTALL_<COMPONENT>` flags
...
beep account or deploying the agent runtime. Explicit targets and
legacy environment selectors are additive and execute in registry order,
so `install forgejo beep` and `install beep forgejo` converge the
same components. `BEEP_INSTALL_FORGEJO=1 install` remains equivalent
...
the beep component. The legacy `BEEP_INSTALL_FORGEJO_RUNNER=1` selector
...
`beep`. `BEEP_INSTALL_LLAMA=1` remains an additive compatibility
...
| -------- | ------- | ------ |
| `BEEP_INSTALL_LLAMA` | `0` | Add the independent `llama` component to selected install targets. |
...
`beep-llama-manager` supports `status`, `start`, `stop`, `restart`, `enable`,
...
### Forgejo server (`BEEP_INSTALL_FORGEJO=1`)
...
| Variable                        | Default                                  | Effect |
| ------------------------------- | ---------------------------------------- | ------ |
| `BEEP_INSTALL_FORGEJO`        | `0`                                      | Set to `1` to install Forgejo + PostgreSQL. |
| `BEEP_INSTALL_FORGEJO_RUNNER` | `0`                                      | Add the co-located Actions runner and its required Forgejo server component. |
...
`/var/lib/beep/components/` by default. Set
`BEEP_COMPONENT_MANIFEST_DIR` to override that directory for tests or
...
`uninstall beep` removes only the beep account/runtime,
...
`--archive` and `--keep-agent` are lifecycle flags for beep removal
...
| Variable             | Default                                        | Effect                                             |
| -------------------- | ---------------------------------------------- | -------------------------------------------------- |
| `BEEP_RECEIPT`     | `1`                                            | Set to `0` to disable the receipt.                 |
| `BEEP_RECEIPT_FILE`| `/var/log/beep/install-receipt.txt`   | Override the receipt file path (absolute).         |
...
All verbs honour the same relevant `BEEP_*` environment variables
documented above. Valid component targets are `beep`, `forgejo`,
`forgejo-runner`, and `llama`.

| Verb        | Effect                                                                |
| ----------- | --------------------------------------------------------------------- |
| `install`   | Full install (default target: `beep`). Safe to re-run.              |
...
```bash
sudo ./scripts/install.sh install beep
...
sudo BEEP_INSTALL_FORGEJO=1 ./scripts/install.sh install
...
`/opt/beep/agent/templates/`, run `sudo ./scripts/install.sh
...
component targets above. They can be combined (e.g. `install beep
...
| `-y`, `--yes`        | Skip the interactive `Type YES` gate (attended scripted runs). Still prompts for any missing inputs unless `BEEP_NONINTERACTIVE=1`. |
| `-q`, `--quiet`      | Only print warnings and errors.                                              |
| `--verbose`, `--debug` | Write shell xtrace to the install transcript (not the console).            |
| `--no-color`         | Disable coloured output (also honours `NO_COLOR` and `BEEP_COLOR=never`).  |
| `--strict`           | Treat preflight warnings as fatal.                                           |
| `--json`             | Emit machine-readable JSON from `verify` / `doctor` (human output is default). |

Colour follows the `BEEP_COLOR=auto|always|never` policy and the
...
| Path                         | Purpose                                                         |
| ---------------------------- | --------------------------------------------------------------- |
| `/opt/beep/skills/`     | Root-owned built-ins, including Ubuntu, file/programming, AI-agent, Hermes, OpenClaw and pi-mono operations. |
| `/etc/beep/skills.d/` | Operator-extensible. Same mode/owner contract as `policy.yaml`. |

Drop additional `*.md` files into `/etc/beep/skills.d/` to
...
| ------------------------- | ---------------------------------------- | ------------------- |
| `BEEP_DIR`              | `/opt/beep`                         | installer, agent    |
| `BEEP_SECRETS`          | `${BEEP_DIR}/secrets/env`              | `server.py`, audit  |
| `BEEP_POLICY`           | `/etc/beep/policy.yaml`         | `policy.py`         |
| `BEEP_AUDIT_LOG`        | `/var/log/beep/audit.log`       | `audit.py`, `beep-audit` |
| `BEEP_AUDIT_VERBOSE`    | *(unset; off)*                           | `audit.py` (opt-in: adds redacted `stdout_preview`/`stderr_preview` to `tool_call` entries to aid pre-release testing and operator debugging) |
| `BEEP_AUDIT_PREVIEW_BYTES` | `2048`                                | `audit.py` (per-stream preview cap when `BEEP_AUDIT_VERBOSE=1`; hard ceiling 16 KiB) |
| `BEEP_HISTORY_DB`       | `/opt/beep/state/conversations.db`  | `history.py`        |
| `BEEP_SKILLS_DIR`       | *(unset)*                                | `skill_loader.py` (extra directory consulted first) |
| `BEEP_NODE`             | `which node`                             | pi-ai bridge spawner |
| `BEEP_PI_AI_BRIDGE`     | `${BEEP_DIR}/agent/pi-ai-bridge.mjs`   | pi-ai bridge spawner (used by tests) |
| `BEEP_PI_MONO_BIN`      | `which pi`                               | `pi_mono.py`        |
| `BEEP_PI_MONO_BRIDGE`   | `${BEEP_DIR}/agent/pi-mono-bridge.mjs` | `pi_mono.py` (used by smoke tests) |
| `BEEP_PI_MONO_LOG_DIR`  | `/opt/beep/state/logs`              | `pi_mono.py`        |
| `BEEP_PI_MONO_SETTINGS` | `/opt/beep/pi/settings.json`        | `pi_mono.py`        |
...
```bash
sudo systemctl enable --now beep-health.timer
```
