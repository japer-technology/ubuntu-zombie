...
Beep is distributed as repository scripts and payload files.
There is no in-place package manager upgrade path yet.

## Recommended process

1. Read `CHANGELOG.md`.
2. Back up `/opt/beep/secrets/env`, `/opt/beep/state/`, and
   `/var/log/beep/audit.log` if you need to keep them.
