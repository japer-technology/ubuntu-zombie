...
| Dependency or release substitution | Pinned Node archive, complete npm v3 graph with registry URLs and SHA-512 integrity, script-free/link-free install, fixed repository workflow identity, checksums, SBOM, attestation, cosign bundles, bounded extraction, and catalogue digests | Reject artifact; retain prior version |
| Failed update leaves a mixed runtime | Locked source revision, private trusted source snapshot, services stopped first, recovery snapshot, reversible staged replacement, health gate, and automatic product rollback | Run doctor; use explicit rollback or verified backup |
