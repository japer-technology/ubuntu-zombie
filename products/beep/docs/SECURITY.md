...
Beep installs a privileged AI Systems Administrator on a
...
- the LLM provider API key in `/opt/beep/secrets/env`;
...
The MVP adds a policy gate (`/etc/beep/policy.yaml`) and an
...
| ------------------- | ----------------------------------------------- |
| LLM provider key    | `sudo /opt/beep/bin/beep-secrets-edit`, then `systemctl restart beep-chat` |
...
Stronger: `sudo systemctl disable --now beep-chat.service`.
...
- `/var/log/beep/audit.log` — JSON-lines record of prompts,
  ...
  time.
- `/opt/beep/bin/beep-audit` — quick view of recent activity.
- `/opt/beep/bin/beep-health` — one-shot health summary.
- `/opt/beep/bin/beep-diagnostics` — bundle for bug reports;
