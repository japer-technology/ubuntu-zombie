...
Beep targets Ubuntu Desktop LTS and installs one local access
...
| Variable | Purpose |
| -------- | ------- |
| `BEEP_USER` | Agent account name; default `beep`. |
| `BEEP_DIR` | Install root; default `/opt/beep`. |
| `BEEP_CHAT_PORT` | Loopback chat port; default `7878`. |
| `BEEP_ADMIN_PASSWORD` | Chat password; default `braaaains`. |
| `BEEP_TTL_DAYS` | Initial Time to Live; default `7`. |
| `BEEP_LOCAL_LLM_MODE` | Local LLM detection mode; default `auto`. |

Provider API keys are added after installation with
`/opt/beep/bin/beep-secrets-edit`.
