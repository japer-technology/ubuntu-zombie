...
How to cut a new release of Beep. For maintainers only.
...
4. **Update `debian/changelog`**: prepend a stanza for the new
   ...
   ```
   beep (2026.06.02.03.59.05) UNRELEASED; urgency=medium
...
8. **Verify the release page**:
   ...
   - Download the assets into one directory, unpack the tarball, then run:
     ...
     ```bash
     mkdir beep-<version>
     tar -xzf beep-<version>.tar.gz -C beep-<version>
     bash beep-<version>/payload/bin/beep-verify-release .
     ```
