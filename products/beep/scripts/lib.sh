...
# lib.sh
# ------
# Shared user-experience helpers for the Beep shell scripts.
#
...
#   1. BEEP_COLOR=always|never|auto  (or the legacy NO_COLOR=1 -> never)
#   2. the --no-color flag a caller maps onto BEEP_COLOR=never
...
BEEP_QUIET="${BEEP_QUIET:-0}"

lib_setup_colors() {
  local mode="${BEEP_COLOR:-auto}"
  local enable=0
  case "${mode}" in
    always) enable=1 ;;
    never)  enable=0 ;;
  ...
  if (( enable )); then
    ...
    # Brand / theme accent palette. The primary highlight is Beep Orchid
    ...
    # BEEP_COLOR=never blank them out and emit no ANSI at all.
...
# --no-color flag re-run lib_setup_colors after setting BEEP_COLOR.
lib_setup_colors
...
log()  { (( BEEP_QUIET )) || printf '%s\n' "$*"; }
info() { (( BEEP_QUIET )) || printf '%s[i]%s %s\n' "${C_CYAN}" "${C_RESET}" "$*"; }
ok()   { (( BEEP_QUIET )) || printf '%s[+]%s %s\n' "${C_GREEN}" "${C_RESET}" "$*"; }
warn() { printf '%s[!]%s %s\n' "${C_YELLOW}" "${C_RESET}" "$*" >&2; }
...
# ---------------------------------------------------------------------------
# Branded UI helpers (Beep Orchid theme)
# ---------------------------------------------------------------------------
...
# brand_wordmark
#   Compact "UBUNTU BEEP" sign, shared by startup screens.
brand_wordmark() {
  (( BEEP_QUIET )) && return 0
  printf '%s' "${C_BRAND}"
  printf '%s\n' \
...
#   The full-dress startup splash: a compact "UBUNTU BEEP" sign in
#   the Beep Orchid palette, framed by a rounded panel that states who the
#   account is, the version, and how to reach it. Used to open the installer
#   the way a polished agent CLI greets you. Honours BEEP_QUIET and the
...
#   or when BEEP_NO_SPINNER=1, it simply runs the command with no
...
run_step() {
  ...
  if [[ ! -t 2 ]] || (( BEEP_QUIET )) || [[ -n "${BEEP_NO_SPINNER:-}" ]]; then
    "$@"
    return $?
  fi
