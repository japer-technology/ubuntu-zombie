#!/usr/bin/env bash
# scripts/build-deb.sh — build a stage-1 .deb of the Beep
# source tree using raw dpkg-deb (no debhelper required).
#
# Output: dist/beep_<version>_all.deb
#
# Usage:
...
usage() {
  cat <<EOF
build-deb.sh — build a stage-1 .deb of the Beep source tree.

Usage:
  bash scripts/build-deb.sh [-h|--help]

Output:
  dist/beep_<version>_all.deb
...
# Payload installed under /usr/share/beep/ so it does not
# conflict with /opt/beep/ (which is created at `beep
...
cat > "${SBIN}/${PKG}" <<EOF
#!/usr/bin/env bash
# beep — thin wrapper around scripts/install.sh.
# Installed by the .deb at /usr/sbin/beep.
set -Eeuo pipefail
INSTALLER="/usr/share/${PKG}/scripts/install.sh"
if [ ! -x "\${INSTALLER}" ]; then
  echo "beep: installer missing at \${INSTALLER}" >&2
