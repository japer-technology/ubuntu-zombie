...
// ships with Node >=18.
//
// The stub script reads BEEP_STUB_PLAN (a JSON array) and emits
...
// When BEEP_STUB_START_OUT is set, record the received `start` frame
...
function recordStart(start) {
  const out = process.env.BEEP_STUB_START_OUT;
  if (!out) return;
  const env = {};
...
const plan = JSON.parse(process.env.BEEP_STUB_PLAN || JSON.stringify([
