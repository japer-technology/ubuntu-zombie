...
def test_whoami_is_provider_independent(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("BEEP_HISTORY_DB", str(tmp_path / "conversations.db"))
    monkeypatch.setenv("BEEP_AUDIT_LOG", str(tmp_path / "audit.log"))
    monkeypatch.setenv("BEEP_PROVIDER", "bogus")
    ...
    try:
        ...
        assert profile["beep_dir"] == "/opt/beep"
