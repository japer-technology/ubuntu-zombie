...
def _beep_policy_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Point the policy loader at the in-tree YAML so tests don't need
    /etc/beep/policy.yaml on the filesystem.
    """
    monkeypatch.setenv("BEEP_POLICY", str(POLICY_FILE))
    # Ensure no stale audit log path leaks from a previous test.
    monkeypatch.delenv("BEEP_AUDIT_LOG", raising=False)
    monkeypatch.delenv("BEEP_AUDIT_VERBOSE", raising=False)
