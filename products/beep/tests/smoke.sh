#!/usr/bin/env bash
# tests/smoke.sh — non-root smoke tests for Beep.
#
# Subcommands:
...
#   noninteractive verify BEEP_NONINTERACTIVE=1 with missing required env
#                  exits with code 64
...
run_python() {
  ...
  PYTHONPATH=payload/agent BEEP_POLICY=payload/etc/policy.yaml python3 - <<'PY'
import platform
import policy
import server
import tempfile
...
for _denied in ("/home", "/root/.ssh/id_rsa",
                "/opt/beep/secrets/env", "/usr/bin/sudo"):
...
# directory when ``BEEP_SKILLS_DIR`` is unset or empty.
import os as _os
from pathlib import Path as _P
_saved = _os.environ.pop("BEEP_SKILLS_DIR", None)
try:
    dirs = _t._skills_dirs()
    assert _P(".") not in dirs and _P("") not in dirs, dirs
    _os.environ["BEEP_SKILLS_DIR"] = ""
    dirs = _t._skills_dirs()
    assert _P(".") not in dirs and _P("") not in dirs, dirs
    _os.environ["BEEP_SKILLS_DIR"] = "/tmp/beep-extra-skills"
    dirs = _t._skills_dirs()
    assert _P("/tmp/beep-extra-skills") in dirs, dirs
finally:
    _os.environ.pop("BEEP_SKILLS_DIR", None)
    if _saved is not None:
        _os.environ["BEEP_SKILLS_DIR"] = _saved
...
assert names == {
    ...
    "troubleshoot", "ubuntu", "users", "virtualization", "web", "beep",
    "zram",
}, names
for s in skills:
...
# and provider selection must honour BEEP_PROVIDER plus the
# expanded key matrix.
import os
...
try:
    ...
    # Unknown BEEP_PROVIDER must fail loudly.
    os.environ["BEEP_PROVIDER"] = "bogus"
    try:
        _pr.provider_from_env()
    except _pr.NoProviderConfigured:
        pass
    else:
        raise SystemExit("unknown BEEP_PROVIDER should raise")
    del os.environ["BEEP_PROVIDER"]
    ...
    # Explicit BEEP_PROVIDER wins over autodetect, but still needs
    # its own key.
    os.environ["BEEP_PROVIDER"] = "gemini"
    try:
        _pr.provider_from_env()
    except _pr.NoProviderConfigured:
        pass
    ...
    # when BEEP_MODEL is not set.
    os.environ["BEEP_PROVIDER"] = "openrouter"
    ...
    try:
        ...
        raise SystemExit("openrouter without BEEP_MODEL should raise")
    os.environ["BEEP_MODEL"] = "anthropic/claude-3.5-sonnet"
    ...
    for k in ("BEEP_PROVIDER", "BEEP_MODEL", "OPENROUTER_API_KEY"):
    ...
    # model so BEEP_MODEL must be set.
    for k in ("BEEP_PROVIDER", "BEEP_MODEL", "GEMINI_API_KEY"):
        os.environ.pop(k, None)
    os.environ["BEEP_PROVIDER"] = "lmstudio"
    ...
    try:
        ...
        raise SystemExit("lmstudio without BEEP_MODEL should raise")
    os.environ["BEEP_MODEL"] = "qwen/qwen3-coder"
...
PY
  ...
  BEEP_HISTORY_DB="${_REACTIVATION_TMP}/conversations.db" \
  BEEP_LIFECYCLE_STATE="${_REACTIVATION_TMP}/lifecycle.json" \
  BEEP_AUDIT_LOG="${_REACTIVATION_TMP}/audit.jsonl" \
  BEEP_POLICY=payload/etc/policy.yaml \
...
Path(os.environ["BEEP_LIFECYCLE_STATE"]).write_text(json.dumps({
...
for name, old_minimum, old_maximum, expected_minimum, expected_maximum in (
    ...
    migration_path = Path(os.environ["BEEP_HISTORY_DB"]).with_name(
        f"migration-{name}.db"
    )
...
with sqlite3.connect(os.environ["BEEP_HISTORY_DB"]) as connection:
...
visible, request, error = server._agent_reactivation_request(
    "I need another turn.\n"
    '<beep-reactivation>{"delay_seconds":5,'
    '"prompt":"Continue the test.","reason":"More work remains.",'
    '"replace_existing":false}</beep-reactivation>'
)
...
try:
    server.pi_mono.run_turn = lambda **kwargs: {
        "final": (
            ...
            '"reason":"Payload outcome."}</beep-reactivation>'
        ),
        "events": [],
        "log_path": None,
    }
...
visible, request, error = server._agent_reactivation_request(
    "Working.\n"
    '<beep-reactivation>{"delay_seconds":1,"prompt":"p"}'
...
visible, request, error = server._agent_reactivation_request(
    "Working.\n```json\n"
    '<beep-reactivation>{"delay_seconds":2,"prompt":"p"}'
    "</beep-reactivation>\n```"
)
assert error is None, error
...
visible, request, error = server._agent_reactivation_request(
    '<beep-reactivation>{"delay_seconds":1,"prompt":"first"}'
    "</beep-reactivation>Middle."
    '<beep-reactivation>{"delay_seconds":3,"prompt":"last"}'
    "</beep-reactivation>"
)
assert error is None, error
...
visible, request, error = server._agent_reactivation_request(
    "Before."
    "<beep-reactivation>```json\n"
    "{'delay_seconds': 4, 'prompt': 'relaxed true', "
    "'replace_existing': false,}\n```"
    "</beep-reactivation>After."
)
assert error is None, error
assert request == {
    "delay_seconds": 4,
...
PY
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    BEEP_PI_MONO_BRIDGE="$(pwd)/tests/fixtures/stub-pi-mono.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
...
PY
    ...
    # resolve the model from BEEP_PROVIDER/BEEP_MODEL (via the same
    ...
    BEEP_PI_MONO_BRIDGE="$(pwd)/tests/fixtures/stub-pi-mono.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    BEEP_STUB_START_OUT="${_START_OUT}" \
    BEEP_PROVIDER="gemini" \
    BEEP_MODEL="gemini-2.0-flash" \
...
PY
    ...
    _MODEL_TMP="$(mktemp -d)"
    BEEP_PI_AI_BRIDGE="$(pwd)/tests/fixtures/stub-pi-ai-bridge.mjs" \
    BEEP_NODE="$(command -v node)" \
    BEEP_HISTORY_DB="${_MODEL_TMP}/conversations.db" \
    BEEP_AUDIT_LOG="${_MODEL_TMP}/audit.log" \
    PYTHONPATH=payload/agent \
      python3 - <<'PY'
import json
import os
...
for k in ("BEEP_PROVIDER", "BEEP_MODEL", "OPENAI_API_KEY",
...
os.environ["BEEP_PROVIDER"] = "openai"
...
# Selecting a known id pins BEEP_MODEL for this process so every
...
if os.environ.get("BEEP_MODEL") != "gpt-4o":
    raise SystemExit("set_active_model must pin BEEP_MODEL")
...
if os.environ.get("BEEP_MODEL") != "gpt-4o":
    raise SystemExit("a rejected selection must not mutate BEEP_MODEL")
...
os.environ["BEEP_PI_MODELS_JSON"] = models_path
...
os.environ["BEEP_PROVIDER"] = "openai"
os.environ.pop("BEEP_MODEL", None)
app = server.App()
...
JS
    ...
    _LM_OUT="$(printf '%s' '{"op":"list_models","provider":"lmstudio"}' \
        | BEEP_PI_MODELS_JSON="${_LM_DIR}/models.json" \
          LMSTUDIO_API_KEY=local \
          node payload/agent/pi-ai-bridge.mjs)"
    kill "${_LM_PID}" 2>/dev/null || true
    BEEP_LM_OUT="${_LM_OUT}" python3 - <<'PY'
import json, os
out = json.loads(os.environ["BEEP_LM_OUT"])
if not out.get("ok"):
...
JS
    ...
    _PI_AI_LOCAL_OUT="$(printf '%s' \
        ...
        | NODE_PATH="${_LM_DIR}/node_modules" \
          ...
          LMSTUDIO_API_KEY=local \
          node payload/agent/pi-ai-bridge.mjs)"
    BEEP_PI_AI_LIST_OUT="${_PI_AI_LIST_OUT}" \
      BEEP_PI_AI_COMPLETE_OUT="${_PI_AI_COMPLETE_OUT}" \
      BEEP_PI_AI_LOCAL_OUT="${_PI_AI_LOCAL_OUT}" python3 - <<'PY'
import json
import os

listed = json.loads(os.environ["BEEP_PI_AI_LIST_OUT"])
if listed != {"ok": True, "models": [{
        "id": "anthropic-model", "name": "Compat model",
        "reasoning": True, "contextWindow": 64000}]}:
    raise SystemExit(f"compat list_models failed: {listed!r}")
completed = json.loads(os.environ["BEEP_PI_AI_COMPLETE_OUT"])
if completed != {"ok": True, "text": "compat completion"}:
    raise SystemExit(f"compat completion failed: {completed!r}")
local = json.loads(os.environ["BEEP_PI_AI_LOCAL_OUT"])
expected_text = (
...
PY
    ...
    BEEP_HISTORY_DB="${_BUDGET_TMP}/conversations.db" \
    BEEP_AUDIT_LOG="${_BUDGET_TMP}/audit.log" \
    BEEP_POLICY="payload/etc/policy.yaml" \
    BEEP_STUB_PLAN='[
      {"type":"tool_call","id":"a","name":"fs.write","args":{"path":"/tmp/beep-budget-1","content":"x"}},
      {"type":"tool_call","id":"b","name":"fs.write","args":{"path":"/tmp/beep-budget-2","content":"x"}},
      {"type":"tool_call","id":"c","name":"fs.write","args":{"path":"/tmp/beep-budget-3","content":"x"}},
...
PY
    ...
    BEEP_PI_MONO_BRIDGE="$(pwd)/tests/fixtures/hang-pi-mono.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
      python3 - <<'PY'
import time
...
PY
    ...
    BEEP_PI_MONO_BRIDGE="$(pwd)/payload/agent/pi-mono-bridge.mjs" \
    BEEP_PI_MONO_BIN="$(pwd)/tests/fixtures/fake-pi-json.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
      python3 - <<'PY'
import time
...
PY
    ...
    BEEP_FAKE_PI_MODE="terminal-answer" \
    BEEP_PI_MONO_BRIDGE="$(pwd)/payload/agent/pi-mono-bridge.mjs" \
    BEEP_PI_MONO_BIN="$(pwd)/tests/fixtures/fake-pi-json.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
...
for mode in ("terminal-answer", "turn-answer"):
    os.environ["BEEP_FAKE_PI_MODE"] = mode
    out = pi_mono.run_turn(
...
PY
    ...
    BEEP_FAKE_PI_MODE="shell-status" \
    BEEP_PI_MONO_BRIDGE="$(pwd)/payload/agent/pi-mono-bridge.mjs" \
    BEEP_PI_MONO_BIN="$(pwd)/tests/fixtures/fake-pi-json.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
...
PY
    ...
    BEEP_FAKE_PI_MODE="partial-error" \
    BEEP_PI_MONO_BRIDGE="$(pwd)/payload/agent/pi-mono-bridge.mjs" \
    BEEP_PI_MONO_BIN="$(pwd)/tests/fixtures/fake-pi-json.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
...
PY
    ...
    _EMPTY_TMP="$(mktemp -d)"
    BEEP_HISTORY_DB="${_EMPTY_TMP}/conversations.db" \
    BEEP_AUDIT_LOG="${_EMPTY_TMP}/audit.log" \
    BEEP_POLICY="payload/etc/policy.yaml" \
    BEEP_FAKE_PI_MODE="silent" \
    BEEP_PI_MONO_BRIDGE="$(pwd)/payload/agent/pi-mono-bridge.mjs" \
    BEEP_PI_MONO_BIN="$(pwd)/tests/fixtures/fake-pi-json.mjs" \
    BEEP_PI_MONO_LOG_DIR="$(mktemp -d)" \
    PYTHONPATH=payload/agent \
...
PY
  ...
  _AUDIT_TMP="$(mktemp -d)"
  BEEP_AUDIT_LOG="${_AUDIT_TMP}/audit.log" \
  PYTHONPATH=payload/agent python3 - <<'PY'
import json
import os
...
os.environ["BEEP_AUDIT_VERBOSE"] = "1"
try:
    audit.log_tool_call(
        tool="shell.run", classification="read_only", decision="executed",
        stdout="visible\nAPI_KEY=secretsesame\nbye",
    ...
    del os.environ["BEEP_AUDIT_VERBOSE"]

path = os.environ["BEEP_AUDIT_LOG"]
...
assert {"beep", "pi-mono", "pi-ai", "python", "node", "sqlite"} <= set(components), info
...
def fake_urlopen(request, timeout):
    ...
    assert request.get_header("User-agent").startswith("beep/")
...
assert checked_components["beep"]["latest"] == "2099.1.2", checked
...
assert f'aria-label="Beep version {server.app_version()}"' in footer, footer
...
PY
  ...
  BEEP_HISTORY_DB="${_STATUS_TMP}/conversations.db" \
  BEEP_LIFECYCLE_STATE="${_STATUS_TMP}/lifecycle.json" \
  BEEP_AUDIT_LOG="${_STATUS_TMP}/audit.log" \
  BEEP_POLICY=payload/etc/policy.yaml \
  PYTHONPATH=payload/agent python3 - <<'PY'
import server
...
PY
  ...
  _CONV_TMP="$(mktemp -d)"
  BEEP_HISTORY_DB="${_CONV_TMP}/conversations.db" \
  BEEP_AUDIT_LOG="${_CONV_TMP}/audit.log" \
  BEEP_POLICY=payload/etc/policy.yaml \
  BEEP_SKILLS_DIR=payload/agent/skills \
  PYTHONPATH=payload/agent python3 - <<'PY'
import atexit
import json
import os
...
_db = os.environ.get("BEEP_HISTORY_DB")
if _db:
...
saved_provider = os.environ.get("BEEP_PROVIDER")
os.environ["BEEP_PROVIDER"] = "bogus"
...
assert profile["beep_dir"] == os.environ.get("BEEP_DIR", "/opt/beep"), profile
assert profile["history_db"] == os.environ["BEEP_HISTORY_DB"], profile
if saved_provider is None:
    os.environ.pop("BEEP_PROVIDER", None)
else:
    os.environ["BEEP_PROVIDER"] = saved_provider
...
PY
  ...
  _LIFE_TMP="$(mktemp -d)"
  BEEP_LIFECYCLE_STATE="${_LIFE_TMP}/lifecycle.json" \
  PYTHONPATH=payload/agent python3 - <<'PY'
import time
import lifecycle
...
st = lifecycle.set_ttl(5)
assert st["dead"] is True, "a dead beep must not be revivable via set_ttl"
...
import json
from pathlib import Path
p = Path(__import__("os").environ["BEEP_LIFECYCLE_STATE"])
...
PY
  ...
  _GATE_TMP="$(mktemp -d)"
  BEEP_HISTORY_DB="${_GATE_TMP}/conversations.db" \
  BEEP_AUDIT_LOG="${_GATE_TMP}/audit.log" \
  BEEP_POLICY=payload/etc/policy.yaml \
  BEEP_LIFECYCLE_STATE="${_GATE_TMP}/lifecycle.json" \
  BEEP_SECRETS="${_GATE_TMP}/secrets/env" \
  PYTHONPATH=payload/agent python3 - <<'PY'
import json
import os
...
run_branding() {
  ...
  grep -Fq 'Beep' payload/bin/beep-chat
...
assert server._provider_banner(
    "openrouter", "model not set (set BEEP_MODEL)"
) == "model not set (set BEEP_MODEL)"
assert server._provider_banner("none", "No provider configured") == (
    "No provider configured"
)
PY
  local out
  out="$(BEEP_COLOR=never ./scripts/install.sh --dry-run)"
  grep -Fq "$first_line" <<<"${out}"
  # A real uninstall run opens with the splash; --help stays concise.
  out="$(BEEP_COLOR=never ./scripts/uninstall.sh --dry-run 2>&1 || true)"
  grep -Fq "$first_line" <<<"${out}"
  out="$(BEEP_COLOR=never ./scripts/uninstall.sh --help)"
...
run_subcommands() {
  ...
  for sub in verify doctor; do
    ...
    for target in beep forgejo forgejo-runner llama; do
      set +e
  ...
  local default_out beep_out forgejo_out runner_out llama_out combined_out
  local forgejo_beep_order_out env_flag_out
  default_out="$(BEEP_COLOR=never ./scripts/install.sh install --dry-run)"
  beep_out="$(BEEP_COLOR=never ./scripts/install.sh --dry-run install beep)"
  [[ "${default_out}" == "${beep_out}" ]] \
    || { echo "FAIL: explicit beep dry-run must match default install" >&2; exit 1; }
  local beep_review
  beep_review="$(sed -n '/^print_parameter_table() {$/,/^}$/p' scripts/install.sh)"
  ! grep -Eq 'Options|review_options' <<<"${beep_review}" \
    || { echo "FAIL: beep parameter review must not ask about options" >&2; exit 1; }

  forgejo_out="$(BEEP_COLOR=never ./scripts/install.sh --dry-run install -- forgejo)"
  ...
  ! grep -q "Agent user:" <<<"${forgejo_out}" \
    || { echo "FAIL: forgejo-only dry-run should not render beep settings" >&2; exit 1; }
  ...
  ! grep -Eq "chat|Time to Live|local LLM|/opt/beep" <<<"${forgejo_out}" \
    || { echo "FAIL: forgejo-only dry-run leaked beep resources" >&2; exit 1; }

  llama_out="$(BEEP_COLOR=never ./scripts/install.sh --dry-run install llama)"
  ...
  ! grep -Eq "Agent user:|Time to Live|/opt/beep" <<<"${runner_out}" \
    || { echo "FAIL: forgejo-runner dry-run leaked beep resources" >&2; exit 1; }

  combined_out="$(BEEP_COLOR=never BEEP_INSTALL_FORGEJO=1 \
    ./scripts/install.sh --dry-run install beep)"
  grep -q "Optional components enabled" <<<"${combined_out}" \
    || { echo "FAIL: legacy Forgejo env flag was not additive" >&2; exit 1; }

  forgejo_beep_order_out="$(BEEP_COLOR=never ./scripts/install.sh --dry-run \
    install forgejo beep)"
  env_flag_out="$(BEEP_COLOR=never BEEP_INSTALL_FORGEJO=1 \
    ./scripts/install.sh --dry-run install)"
  [[ "${combined_out}" == "${forgejo_beep_order_out}" \
    && "${combined_out}" == "${env_flag_out}" ]] \
    || { echo "FAIL: combined targets and legacy flag must resolve identically" >&2; exit 1; }

  # Forgejo-only selection must not validate beep-only settings.
  expect_exit_code 0 env BEEP_USER=INVALID BEEP_CHAT_PORT=not-a-port \
    BEEP_TTL_DAYS=invalid BEEP_NONINTERACTIVE=1 \
    ./scripts/install.sh install forgejo --dry-run
  expect_exit_code 64 env BEEP_RECEIPT=0 BEEP_NONINTERACTIVE=1 \
    ./scripts/install.sh install forgejo --yes

  # The extracted Forgejo hook must not depend on beep runtime state.
  local forgejo_hook
  forgejo_hook="$(sed -n \
  ...
  ! grep -Eq 'AGENT_USER|AGENT_HOME|CHAT_PORT|TTL_DAYS|LOCAL_LLM|BEEP_ETC|/opt/beep' \
    <<<"${forgejo_hook}" \
    || { echo "FAIL: install_forgejo references beep-owned state" >&2; exit 1; }
  ...
  for sub in install verify doctor repair uninstall; do
    ...
    expect_exit_code 2 ./scripts/install.sh "${sub}" beep beep
  ...
  expect_exit_code 0 ./scripts/install.sh uninstall beep --archive --keep-agent --dry-run
}
...
run_manifest() {
  ...
  local scratch_root bogus_beep_dir verify_beep_dir out manifest_dir
  scratch_root="$(pwd)/tests/.smoke-manifest.$$"
  rm -rf -- "${scratch_root}"
  mkdir -p "${scratch_root}"
  trap 'rm -rf -- "'"${scratch_root}"'"' RETURN
  bogus_beep_dir="${scratch_root}/missing-beep-root"
  verify_beep_dir="${scratch_root}/verify-beep-root"

  manifest_dir="${scratch_root}/valid-beep"
  mkdir -p "${manifest_dir}"
  cat > "${manifest_dir}/beep" <<'EOF_MANIFEST'
format=1
component=beep
beep_version=test
...
suboptions=
EOF_MANIFEST
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    BEEP_DIR="${bogus_beep_dir}" ./scripts/install.sh doctor --json)"
  grep -Eq '"component"[[:space:]]*:[[:space:]]*"beep"' <<<"${out}" \
    || { echo "FAIL: doctor --json did not discover beep manifest" >&2; exit 1; }
  ...
  mkdir -p "${verify_beep_dir}/bin" "${verify_beep_dir}/secrets"
  cat > "${verify_beep_dir}/bin/verify" <<'EOF_VERIFY'
#!/usr/bin/env bash
echo "STALE VERIFIER RAN" >&2
exit 99
EOF_VERIFY
  chmod +x "${verify_beep_dir}/bin/verify"
  printf '%s\n' 'BEEP_ADMIN_PASSWORD_HASH=pbkdf2_sha256$600000$salt$digest' \
    > "${verify_beep_dir}/secrets/env"
  chmod 600 "${verify_beep_dir}/secrets/env"

  set +e
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    BEEP_USER="verify-missing" BEEP_DIR="${verify_beep_dir}" \
...
EOF_BAD_MANIFEST
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    BEEP_DIR="${bogus_beep_dir}" ./scripts/install.sh doctor --json 2>/dev/null)"
  ! grep -Eq '"component"[[:space:]]*:[[:space:]]*"beep"' <<<"${out}" \
    || { echo "FAIL: malformed duplicate-key manifest should be ignored" >&2; exit 1; }

  manifest_dir="${scratch_root}/unknown-key"
  mkdir -p "${manifest_dir}"
  cat > "${manifest_dir}/beep" <<'EOF_UNKNOWN_MANIFEST'
format=1
component=beep
beep_version=test
converged_utc=2026-01-01T00:00:00Z
...
EOF_MANIFEST
  ...
  out="$(BEEP_COLOR=never BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
  ...
  ! grep -q "beep-chat" <<<"${out}" \
    || { echo "FAIL: forgejo-only dry-run should not include beep service cleanup" >&2; exit 1; }
...
EOF_FAKE_PSQL
  ...
  out="$(BEEP_COLOR=never ./scripts/uninstall.sh beep --dry-run 2>&1 || true)"
  grep -q "beep" <<<"${out}" \
    || { echo "FAIL: beep-only dry-run should mention beep cleanup" >&2; exit 1; }
  ! grep -q "forgejo.service" <<<"${out}" \
    || { echo "FAIL: beep-only dry-run should not include Forgejo cleanup" >&2; exit 1; }
  ...
  expect_exit_code 0 ./scripts/uninstall.sh beep --archive --keep-agent --dry-run

  out="$(BEEP_COLOR=never ./scripts/uninstall.sh --dry-run 2>&1 || true)"
  grep -q "forgejo" <<<"${out}" \
    || { echo "FAIL: no-target dry-run should mention Forgejo selection" >&2; exit 1; }
  grep -q "beep" <<<"${out}" \
    || { echo "FAIL: no-target dry-run should mention beep cleanup" >&2; exit 1; }
...
suboptions=
EOF_MANIFEST
  BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" ./scripts/uninstall.sh beep --dry-run >/dev/null 2>&1 || true
  [[ -f "${manifest_dir}/beep" ]] \
    || { echo "FAIL: dry-run uninstall must not remove beep manifest" >&2; exit 1; }
...
EOF_MANIFEST
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    ./scripts/uninstall.sh beep --dry-run 2>&1 || true)"
  grep -q "forgejo" <<<"${out}" \
    || { echo "FAIL: targeted beep dry-run should warn when Forgejo manifest remains" >&2; exit 1; }
...
EOF_MANIFEST
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    BEEP_DIR="${bogus_beep_dir}" ./scripts/install.sh verify --json 2>/dev/null || true)"
  grep -Eq '"component"[[:space:]]*:[[:space:]]*"forgejo"' <<<"${out}" \
    || { echo "FAIL: verify --json did not discover forgejo manifest" >&2; exit 1; }

  rm -rf -- "${bogus_beep_dir}"
  mkdir -p "${bogus_beep_dir}"
  out="$(BEEP_COMPONENT_MANIFEST_DIR="${scratch_root}/no-manifests" \
    BEEP_USER="verify-missing" BEEP_DIR="${bogus_beep_dir}" \
    ./scripts/install.sh verify --json 2>&1 || true)"
  grep -Eq '"component"[[:space:]]*:[[:space:]]*"beep"' <<<"${out}" \
    || { echo "FAIL: verify --json should report partial legacy beep installs" >&2; exit 1; }
...
EOF_MANIFEST
  ...
  mkdir -p "${manifest_dir}"
  # File named 'beep' but declares component=forgejo
  cat > "${manifest_dir}/beep" <<'EOF_MANIFEST'
format=1
component=forgejo
beep_version=test
...
EOF_MANIFEST
  ...
  # --- Path-traversal in BEEP_COMPONENT_MANIFEST_DIR (install.sh) ----
  local _rc_inst _rc_uninst
  set +e
  BEEP_COMPONENT_MANIFEST_DIR='/var/lib/../etc' \
  ...
  set -e
  [[ "${_rc_inst}" -eq 2 ]] \
    || { echo "FAIL: install.sh should reject BEEP_COMPONENT_MANIFEST_DIR with traversal (exit 2, got ${_rc_inst})" >&2; exit 1; }
  [[ "${_rc_uninst}" -eq 2 ]] \
    || { echo "FAIL: uninstall.sh should reject BEEP_COMPONENT_MANIFEST_DIR with traversal (exit 2, got ${_rc_uninst})" >&2; exit 1; }

  # --- verify in mixed mode includes beep component identity ----------
  manifest_dir="${scratch_root}/verify-mixed-beep-forgejo"
  mkdir -p "${manifest_dir}"
  cat > "${manifest_dir}/beep" <<'EOF_MANIFEST'
format=1
component=beep
...
EOF_MANIFEST
  ...
  grep -Eq '"component"[[:space:]]*:[[:space:]]*"beep"' <<<"${out}" \
    || { echo "FAIL: mixed verify --json must include beep component checks" >&2; exit 1; }
  grep -Eq '"component"[[:space:]]*:[[:space:]]*"forgejo"' <<<"${out}" \
    || { echo "FAIL: mixed verify --json must include forgejo component checks" >&2; exit 1; }
  # Mixed beep checks should include more than just the verifier script:
  # at minimum user and install_root checks must be present.
  grep -Eq '"id"[[:space:]]*:[[:space:]]*"user"' <<<"${out}" \
    || { echo "FAIL: mixed verify --json must include a 'user' check for beep" >&2; exit 1; }

  # --- Lifecycle isolation: forgejo-only uninstall leaves beep manifest
...
suboptions=
EOF_MANIFEST
  BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    ./scripts/uninstall.sh forgejo --dry-run >/dev/null 2>&1 || true
  [[ -f "${manifest_dir}/beep" ]] \
    || { echo "FAIL: forgejo-only dry-run must not remove beep manifest" >&2; exit 1; }

  # --- beep-only uninstall leaves forgejo manifest --------------------
  BEEP_COMPONENT_MANIFEST_DIR="${manifest_dir}" \
    ./scripts/uninstall.sh beep --dry-run >/dev/null 2>&1 || true
  [[ -f "${manifest_dir}/forgejo" ]] \
    || { echo "FAIL: beep-only dry-run must not remove forgejo manifest" >&2; exit 1; }
...
run_bad_usage() {
  ...
  expect_exit_code 2 env 'BEEP_USER=bad user' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_USER=root' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_USER=bad-' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_USER=bad_' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_DIR=relative/path' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_DIR=/tmp/beep;touch /tmp/install-path-pwn' ./scripts/install.sh doctor
  expect_exit_code 2 env 'LOG_FILE=relative.log' ./scripts/install.sh doctor
  expect_exit_code 2 env 'LOG_FILE=/tmp/beep log' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_RECEIPT=1' 'BEEP_RECEIPT_FILE=relative-receipt.txt' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_CHAT_PORT=70000' ./scripts/install.sh doctor
  # FIX-2-01: uninstall.sh must validate BEEP_USER / paths *before*
  # any side-effecting command runs (so a smoke run as non-root still
  # exits 2 rather than 1).
  expect_exit_code 2 env 'BEEP_USER=beep;touch /tmp/beep-pwn' ./scripts/uninstall.sh --dry-run
  expect_exit_code 2 env 'BEEP_USER=root' ./scripts/uninstall.sh --dry-run
  expect_exit_code 2 env 'BEEP_DIR=relative/path' ./scripts/uninstall.sh --dry-run
  expect_exit_code 2 env 'BEEP_DIR=/tmp/beep;touch /tmp/uninstall-path-pwn' ./scripts/uninstall.sh --dry-run
  expect_exit_code 2 env 'BACKUP_DIR=relative/path' ./scripts/uninstall.sh --dry-run
  expect_exit_code 2 env 'BACKUP_DIR=/tmp/beep backup' ./scripts/uninstall.sh --dry-run
  [[ ! -e /tmp/beep-pwn ]] || { echo "FAIL: uninstall.sh BEEP_USER injection created /tmp/beep-pwn" >&2; exit 1; }
  [[ ! -e /tmp/install-path-pwn ]] || { echo "FAIL: install.sh BEEP_DIR injection created /tmp/install-path-pwn" >&2; exit 1; }
  [[ ! -e /tmp/uninstall-path-pwn ]] || { echo "FAIL: uninstall.sh BEEP_DIR injection created /tmp/uninstall-path-pwn" >&2; exit 1; }
  ...
  grep -Fq 'remove_tree_checked "${BEEP_DIR}" "${BEEP_DIR}"' scripts/uninstall.sh
...
run_noninteractive() {
  ...
  # (FIX-1-13).
  ./scripts/install.sh --help | grep -q BEEP_NONINTERACTIVE
  ...
  base_out="$(BEEP_COLOR=never ./scripts/install.sh install --dry-run)"
  if grep -q "Optional components enabled" <<<"${base_out}"; then
    echo "FAIL: default dry-run must not mention optional components" >&2
    exit 1
  fi
  fj_out="$(BEEP_COLOR=never BEEP_NONINTERACTIVE=1 BEEP_INSTALL_FORGEJO=1 \
    BEEP_INSTALL_FORGEJO_RUNNER=1 ./scripts/install.sh install --dry-run)"
  ...
  local llama_out
  llama_out="$(BEEP_COLOR=never BEEP_NONINTERACTIVE=1 \
  ...
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=2' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_HTTP_PORT=70000' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_DB_NAME=Bad;Name' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_ADMIN_USER=-bad' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_ADMIN_USER=a-' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_DB_USER=bad_' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_ADMIN_PASSWORD=short' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_DB_PASSWORD=short' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_VERSION=not.a.version!' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_FORGEJO=1' 'FORGEJO_RUNNER_LABELS=bad label' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_LLAMA=2' ./scripts/install.sh doctor
  expect_exit_code 0 env 'LLAMA_PORT=8080' 'BEEP_NONINTERACTIVE=1' \
    ./scripts/install.sh install llama --dry-run
  expect_exit_code 2 env 'BEEP_INSTALL_LLAMA=1' 'LLAMA_PORT=8081' ./scripts/install.sh doctor
  expect_exit_code 2 env 'BEEP_INSTALL_LLAMA=1' 'LLAMA_CONTEXT_SIZE=nope' ./scripts/install.sh doctor
  # The approved default model has a 2048-token catalogue ceiling.
  expect_exit_code 2 env 'BEEP_INSTALL_LLAMA=1' 'LLAMA_CONTEXT_SIZE=4096' ./scripts/install.sh doctor
}

run_diagnostics() {
  echo "[smoke] beep-diagnostics best-effort"
  # FIX-3-24: beep-diagnostics must finish and write its tarball
  ...
  local td
  td="$(mktemp -d)"
  if ! TMPDIR="${td}" bash payload/bin/beep-diagnostics >/dev/null 2>&1; then
    rm -rf "${td}"
    echo "FAIL: beep-diagnostics exited non-zero (best-effort capture regressed)" >&2
    exit 1
  fi
  local -a tarballs
  mapfile -t tarballs < <(find "${td}" -maxdepth 1 -name 'beep-diagnostics-*.tar.gz' -print)
  if [[ "${#tarballs[@]}" -ne 1 ]]; then
    rm -rf "${td}"
    echo "FAIL: beep-diagnostics must produce exactly one tarball (found ${#tarballs[@]})" >&2
    exit 1
  fi
  local tarball
  tarball="${tarballs[0]}"
  if [[ ! -s "${tarball}" ]]; then
    rm -rf "${td}"
    echo "FAIL: beep-diagnostics produced an empty tarball" >&2
    exit 1
  fi
  # The staging directory must be cleaned up by the EXIT trap, leaving
  # only the tarball behind.
  if find "${td}" -maxdepth 1 -type d -name 'beep-diagnostics-*' | grep -q .; then
    rm -rf "${td}"
    echo "FAIL: beep-diagnostics left its staging directory behind" >&2
    exit 1
  fi
  rm -rf "${td}"

  echo "[smoke] beep-health timer warn-only mode"
  td="$(mktemp -d)"
...
EOF
  ...
  if PATH="${td}:${PATH}" BEEP_DIR="${td}/missing" \
      bash payload/bin/beep-health >/dev/null 2>&1; then
    rm -rf "${td}"
    echo "FAIL: beep-health should exit non-zero for manual failed checks" >&2
    exit 1
  fi
  if ! PATH="${td}:${PATH}" BEEP_DIR="${td}/missing" BEEP_HEALTH_WARN_ONLY=1 \
      bash payload/bin/beep-health >/dev/null 2>&1; then
    rm -rf "${td}"
    echo "FAIL: beep-health timer warn-only mode should not fail the systemd unit" >&2
    exit 1
  fi
  rm -rf "${td}"
}
...
run_standards() {
  ...
  local required=(
    ...
    payload/bin/beep-verify-release
    debian/control.in
    debian/postinst
    debian/prerm
    debian/copyright
  ...
  # installer can deploy them to /opt/beep/skills/.
  local s
  ...
  grep -q "__BEEP_DIR__" payload/systemd/beep-chat.service \
    || { echo "chat systemd template must keep __BEEP_DIR__ placeholder" >&2; exit 1; }
  grep -q "ExecStart=__BEEP_DIR__/bin/beep-health" payload/systemd/beep-health.service \
    || { echo "health systemd template must use __BEEP_DIR__ placeholder" >&2; exit 1; }
  grep -q "BEEP_HEALTH_WARN_ONLY=1" payload/systemd/beep-health.service \
...
EOF
  ...
  grep -q 'GROQ|LMSTUDIO' payload/bin/beep-health \
...
EOF
  bash -c "${model_helper}
    ...
    printf '%s\n' 'BEEP_MODEL=installed-model' >> \"\${BEEP_DIR}/secrets/env\"
    model_selection_configured
    : > \"\${BEEP_DIR}/secrets/env\"
    BEEP_OPENAI_MODEL=environment-model
  ...
  BEEP_LIFECYCLE_STATE="${lifecycle_state}" \
...
EOF
  ...
  grep -q "BEEP_INSTALL_FORGEJO" scripts/uninstall.sh 2>/dev/null \
  ...
  python3 payload/bin/beep-llama-manager --help >/dev/null
  python3 - <<'PY'
...
loader = importlib.machinery.SourceFileLoader(
    "llama_manager", "payload/bin/beep-llama-manager"
)
...
test = r'''
const marker = "<beep-reactivation>";
...
if (!brandWordmark("").includes("UBUNTU BEEP")) {
...
PY
  ...
  grep -q "s|__BEEP_DIR__|\\\${BEEP_DIR}|g" scripts/install.sh \
    || { echo "install.sh must render __BEEP_DIR__ in systemd units" >&2; exit 1; }
...
run_flags() {
  ...
  ./scripts/install.sh --help | grep -q "BEEP_INSTALL_FORGEJO"
  ./scripts/install.sh --help | grep -q "BEEP_INSTALL_LLAMA"
  ...
  # Exceptions to the exit-2 check: beep-audit's -n takes a numeric
  # value it validates itself, and beep-verify-release accepts an arbitrary
  # positional release directory, so a bogus flag is not "unknown" to it.
  local helpers=(
    payload/bin/beep-diagnostics
    payload/bin/beep-health
    payload/bin/beep-secrets-edit
    payload/bin/beep-setup-venv
    payload/bin/beep-chat
    scripts/build-deb.sh
    scripts/verify-bridge-pins.sh
  )
  local helper
  for helper in "${helpers[@]}" payload/bin/beep-audit payload/bin/beep-verify-release; do
    bash "${helper}" --help >/dev/null \
      || { echo "FAIL: ${helper} --help did not exit 0" >&2; exit 1; }
  done
  for helper in "${helpers[@]}" payload/bin/beep-audit; do
  ...
  for component in beep forgejo forgejo-runner llama; do
