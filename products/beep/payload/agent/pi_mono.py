...
For development and CI, ``BEEP_PI_MONO_BRIDGE`` may point at any
...
DEFAULT_LOG_DIR = Path(os.environ.get(
    "BEEP_PI_MONO_LOG_DIR", "/var/lib/beep/runtime/logs"))
DEFAULT_SETTINGS_PATH = Path(os.environ.get(
    "BEEP_PI_MONO_SETTINGS", "/opt/beep/pi/settings.json"))
...
# clean error (Python < bridge BEEP_PI_MONO_IDLE_TIMEOUT < client
...
def _bridge_argv() -> list[str]:
    explicit = os.environ.get("BEEP_PI_MONO_BRIDGE")
    if explicit:
    ...
    if node is None:
        raise BridgeError(
            ...
            "or set BEEP_PI_MONO_BRIDGE to point at a stub."
        )
...
def run_turn(
    *,
    ...
    to ``BEEP_PI_MONO_TIMEOUT`` then :data:`DEFAULT_TURN_TIMEOUT`; a
    ...
    def _watchdog() -> None:
        while not stop_watchdog.wait(0.5):
            # An explicit operator stop wins if it races the idle deadline.
...
def render_settings(*, tool_names_list: Iterable[str]) -> dict[str, Any]:
    ...
    writes to ``/opt/beep/pi/settings.json``."""
    return {
        "mode": "rpc",
        "noBuiltinTools": True,
