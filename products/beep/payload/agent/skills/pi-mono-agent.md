...
- Distinguish the embedded runtime from a user's standalone `pi`
  installation. Beep pins `@earendil-works/pi-coding-agent`, renders
  settings under `/opt/beep/pi/`, and starts it through
  `/opt/beep/agent/pi-mono-bridge.mjs`.
- The Python chat service owns provider/model selection and starts the Node
  ...
  listening socket.
- Beep disables pi's built-in tools. Every effective tool call must
...
- Per-turn logs live under `/var/lib/beep/runtime/logs/`; session/checkpoint
  state lives under `/var/lib/beep/runtime/pi-mono-sessions/`. Redact prompts,
  tool arguments and provider errors before sharing diagnostics.
- Provider credentials come from Beep's root-owned secrets flow.
...
- Restarting `beep-chat.service` terminates the active conversation.
