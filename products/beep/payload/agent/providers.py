...
``/etc/beep/secrets/env``)::
    ...
    xai         XAI_API_KEY
    openrouter  OPENROUTER_API_KEY    (BEEP_MODEL must be set)
    mistral     MISTRAL_API_KEY
    groq        GROQ_API_KEY
    lmstudio    LMSTUDIO_API_KEY      (BEEP_MODEL must be set)
...
class NoProviderConfigured(ProviderError):
    """Raised when no provider key (or an unknown ``BEEP_PROVIDER``) is set."""
...
# default model used when ``BEEP_MODEL`` is unset, env var for a
...
_PI_AI_PROVIDERS: tuple[_ProviderSpec, ...] = (
    ...
    _ProviderSpec("anthropic",  "ANTHROPIC_API_KEY",  "claude-3-5-sonnet-latest",
                  "BEEP_ANTHROPIC_MODEL"),
    _ProviderSpec("gemini",     "GEMINI_API_KEY",     "gemini-2.0-flash",
                  "BEEP_GEMINI_MODEL", pi_provider="google"),
...
class BaseProvider:
    ...
    def __init__(self, spec: _ProviderSpec, model: str | None = None) -> None:
        ...
        # explicit arg > BEEP_MODEL > provider-specific override > default.
...
def provider_status() -> tuple[str, str]:
    ...
    def _ok(spec: _ProviderSpec) -> tuple[str, str]:
        ...
        if not model:
            return (spec.name, "model not set (set BEEP_MODEL" + (f" or {spec.model_env}" if spec.model_env else "") + ")")
        address = lmstudio_address() if spec.name == "lmstudio" else None
        return (spec.name, f"model {model}" + (f" at {address}" if address else ""))

    explicit = (os.environ.get("BEEP_PROVIDER") or "").strip().lower()
    if explicit:
...
def _resolve_spec(name: str | None = None) -> _ProviderSpec:
    ...
    2. ``BEEP_PROVIDER`` environment variable.
    ...
    """
    explicit = (name or os.environ.get("BEEP_PROVIDER", "")).strip().lower()
    if explicit:
        spec = _PROVIDER_BY_NAME.get(explicit)
        if spec is None:
            raise NoProviderConfigured(
...
def current_model(name: str | None = None) -> str | None:
    ...
    (``BEEP_MODEL`` > provider override env > registry default).
...
def _models_json_path() -> Path:
    explicit = os.environ.get("BEEP_PI_MODELS_JSON")
    if explicit:
...
def scan_lmstudio(
    ...
    the local /24 and on loopback. The Beep-private llama.cpp port remains
    loopback-only.
    """
    try:
        scan_port = int(port if port is not None
                        else os.environ.get("BEEP_LLM_SCAN_PORT", "1234"))
    except (TypeError, ValueError) as exc:
        raise ProviderError("BEEP_LLM_SCAN_PORT must be a TCP port.") from exc
    if not (1 <= scan_port <= 65535):
        raise ProviderError("BEEP_LLM_SCAN_PORT must be between 1 and 65535.")
    try:
        subnet = (
...
def activate_lmstudio(server: dict) -> tuple[str, str, str]:
    ...
    current = os.environ.get("BEEP_MODEL", "")
...
def set_active_model(model: str, name: str | None = None) -> tuple[str, str]:
    ...
    Sets ``BEEP_MODEL`` in the current process environment so every
