"""Time-to-live (TTL) kill switch for the Beep.

The Beep Systems Administrator is a privileged, root-capable
account. To bound the window in which it can act, a fresh install is
given a *Time to Live*: a future moment after which the beep is
...
cannot revive the beep. Routine reinstalls preserve the tombstone;
only an explicit ``initialize`` call resets the lifecycle.

State lives in a small JSON file (``/var/lib/beep/runtime/lifecycle.json``
by default, overridable with ``BEEP_LIFECYCLE_STATE``)::
...
STATE_PATH = Path(
    os.environ.get("BEEP_LIFECYCLE_STATE", "/var/lib/beep/runtime/lifecycle.json")
)
...
def _state_path() -> Path:
    ...
    return Path(
        os.environ.get("BEEP_LIFECYCLE_STATE", str(STATE_PATH))
    )
...
def set_ttl(days: float) -> dict[str, Any]:
    ...
    expiry, so it never shortens an existing countdown. A dead beep
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Manage the Beep time-to-live kill switch."
    )
    ...
    sub.add_parser("die", help="Permanently disable the beep now.")
