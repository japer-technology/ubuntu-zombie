...
// error and exits; smoke tests override it via $BEEP_PI_MONO_BRIDGE.
//
...
async function run() {
  ...
  const piBin = process.env.BEEP_PI_MONO_BIN || "pi";
  ...
  // Disabled when BEEP_PI_MONO_IDLE_TIMEOUT <= 0.
  const idleTimeoutMs = (() => {
    const raw = Number(process.env.BEEP_PI_MONO_IDLE_TIMEOUT);
