...
``BEEP_AUDIT_VERBOSE=1``
    Adds redacted ``stdout_preview`` / ``stderr_preview`` snippets to
    ``tool_call`` entries (capped to ``BEEP_AUDIT_PREVIEW_BYTES``,
...
``BEEP_AUDIT_PREVIEW_BYTES=N``
    Override the per-stream preview cap (only honoured when
    ``BEEP_AUDIT_VERBOSE=1``). Hard ceiling is 16 KiB to keep the
    log bounded.
...
def _verbose() -> bool:
    ...
    return _truthy(os.environ.get("BEEP_AUDIT_VERBOSE"))


def _preview_cap() -> int:
    raw = os.environ.get("BEEP_AUDIT_PREVIEW_BYTES")
    if not raw:
...
def log_tool_call(
    ...
    **extra: Any,
    ...
    When ``BEEP_AUDIT_VERBOSE=1`` is set, a redacted preview of each
    stream (capped at ``BEEP_AUDIT_PREVIEW_BYTES``, default 2048) is
