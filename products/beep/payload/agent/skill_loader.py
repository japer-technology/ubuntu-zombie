...
* ``/opt/beep/skills/``        — root-owned, ships with the package.
* ``/etc/beep/skills.d/``  — operator-extensible, same
  mode/owner contract as ``/etc/beep/policy.yaml``.
...
def load_skills(dirs: Iterable[Path] | None = None) -> list[Skill]:
    ...
    in ``/etc/beep/skills.d/`` cannot quietly shadow the
