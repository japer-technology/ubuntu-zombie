...
Desktop LTS machine to turn it into an Beep: a private,
...
body of the beep; `scripts/install.sh` is the ritual that animates
...
flowchart TD
    ...
    subgraph host["target host (after scripts/install.sh)"]
        opt_agent["/opt/beep/agent/<br/>(root-owned sources +<br/>pi-ai / pi-mono bridges)"]
        opt_bin["/opt/beep/bin/<br/>+ symlinks in /usr/local/bin"]
        opt_skills["/opt/beep/skills/<br/>(built-in catalogue)"]
        opt_pi["/opt/beep/pi/<br/>(rendered settings + system prompt)"]
        etc_dst["/etc/beep/policy.yaml<br/>+ /etc/beep/skills.d/"]
        sys_dst["/etc/systemd/system/<br/>beep-*.{service,timer}"]
        log_dst["/etc/logrotate.d/beep"]
        state["/opt/beep/state/<br/>conversations.db + per-turn logs"]
    end
...
Placeholders such as `__AGENT_USER__`, `__BEEP_DIR__`, and
...
|--------------------------------|------------------------------------------|------------------------|
| `agent/*.py`, `agent/*.mjs`    | `/opt/beep/agent/`                  | `AGENT_USER` 644       |
| `agent/templates/*`            | `/opt/beep/agent/templates/` + rendered into `/opt/beep/pi/` | mixed |
| `agent/skills/*.md`            | `/opt/beep/skills/`                 | `root` 644             |
| `bin/*`                        | `/opt/beep/bin/` (+ `/usr/local/bin` symlinks) | `AGENT_USER` 755 |
| `etc/policy.yaml`              | `/etc/beep/policy.yaml`         | `root` 644 (preserved if present) |
| `systemd/*`                    | `/etc/systemd/system/`                   | `root` 644             |
| `logrotate/beep`      | `/etc/logrotate.d/beep`         | `root` 644             |
...
| [`providers.py`](agent/providers.py) | Single source of truth for provider + model selection, resolved from `BEEP_PROVIDER`, `BEEP_MODEL`, and the matching `*_API_KEY`. |
...
| [`templates/`](agent/templates) | The chat UI (`index.html`) plus `settings.json.tmpl` and `APPEND_SYSTEM.md.tmpl`, rendered by the installer into `/opt/beep/pi/`. |
...
| Helper | Purpose |
|--------|---------|
| [`beep-chat`](bin/beep-chat) | Print the chat URL, an SSH-tunnel example, and every debugging entry point — the single command to discover the diagnostic surface. |
| [`beep-health`](bin/beep-health) | One-shot health summary (also run periodically by the health timer). |
| [`beep-audit`](bin/beep-audit) | Pretty-print recent audit-log entries (`-n`, `--all`, `-t TYPE`, `--follow`). |
| [`beep-diagnostics`](bin/beep-diagnostics) | Bundle logs and state into a tarball for bug reports, redacting secrets first. |
| [`beep-secrets-edit`](bin/beep-secrets-edit) | Safely edit `/opt/beep/secrets/env`, re-asserting `0600` and ownership on exit. |
| [`beep-setup-venv`](bin/beep-setup-venv) | Provision the unprivileged agent Python venv and Playwright/Chromium (run as the agent user, idempotent). |
| [`beep-verify-release`](bin/beep-verify-release) | Verify a downloaded release: `SHA256SUMS`, cosign signatures, and SLSA provenance. |

> ⚠️ **Do not run the mutating helpers** (`beep-secrets-edit`,
> `beep-setup-venv`) or the installer outside a disposable Ubuntu
...
*preserves* an existing `/etc/beep/policy.yaml` rather than
overwriting it.
...
| Unit | Role |
|------|------|
| `beep-chat.service` | Runs `server.py` as `AGENT_USER` on `127.0.0.1:7878`. **Sandboxing is deliberately off** (`ProtectSystem=false`, no `NoNewPrivileges`): the agent is a full Systems Administrator that elevates via passwordless `sudo`, so a private mount namespace or `NoNewPrivileges` would break approved work. The policy gate — not a systemd sandbox — is the security boundary. |
| `beep-health.service` | Oneshot wrapper around `bin/beep-health`. |
| `beep-health.timer` | Runs the health check every 15 minutes. |

---

## `logrotate/beep` — log hygiene

Rotation rules for the audit log, the install log, and the per-turn
`pi-mono` logs under `/opt/beep/state/logs/`. The `__AGENT_USER__`
