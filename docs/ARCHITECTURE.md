...
│   │   ├── auth.py                   # chat-UI password gate (PBKDF2 hash)
│   │   ├── lifecycle.py              # Time-to-Live kill switch + tombstone
...
| `auth.py`       | Chat-UI password gate. Stores only a PBKDF2-SHA256 hash (`ZOMBIE_ADMIN_PASSWORD_HASH`); `check_password` uses `hmac.compare_digest`. The gate is opt-in: when no hash is configured `auth_required()` is false (so headless tests/tools still work). The installer computes the hash from the operator's chosen password (default `livelongandprosper`) via the module's CLI, reading the plaintext from stdin to keep it off the command line. |
| `lifecycle.py`  | Time-to-Live kill switch. Persists `{expires_at, dead}` in `/opt/ai-zombie/state/lifecycle.json` (override `ZOMBIE_LIFECYCLE_STATE`). `status()` trips and durably records a tombstone once the TTL elapses; `set_ttl(days)` extends from the current expiry (refuses to revive a dead zombie); `kill()` is the `/ttl --die` path; `initialize(days)` is the only path that clears a tombstone and is run by the installer on every (re)install. Once dead the server refuses prompts until a reinstall. |
